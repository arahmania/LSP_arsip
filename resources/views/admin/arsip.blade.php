                            ...
                            <span>Cari surat
                                <label>
                                    <input type="search" name="q" class="form-control form-control-sm"
                                        placeholder="Nomor Surat atau Judul" aria-controls="dataTable">
                                </label>
                        ...
                        <tbody>
                            @foreach($data as $dataArsip)
                                <tr>
                                    ...
                                    <td>
                                        <form action="{{ route('arsip.hapus', $dataArsip->id) }}" method="post">
                                            @csrf
                                            <button class="btn btn-danger px-3"
                                                onClick="return confirm('Yakin Hapus Data?')">Hapus</button>
                                            <a href="{{ asset($dataArsip->file_surat) }}"
                                                class="btn btn-primary px-4 text mb-1" download>Unduh</a>
                                            <a href="{{ route('arsip.lihat-surat', $dataArsip->id) }}"
                                                class="btn btn-info" target="_blank">Lihat</a>
                                        </form>
                                    </td>
                                </tr>
                            @endforeach
